...
const getStream = function ({ link, providerContext, }) {
    ...
    return axios
        ...
        $(".server-item").each((_, el) => {
            ...
            if (!sourceId)
                return;
            serverTasks.push(resolveSource(sourceId, type || "sub", axios, commonHeaders));
        });
        return Promise.all(serverTasks).then((all) => all.flat().filter(Boolean));
    })
        .catch(() => []);
};
exports.getStream = getStream;
// ---------------- RESOLVE SOURCE → M3U8 ----------------
function resolveSource(sourceId, type, axios, headers) {
    ...
    return axios
        ...
        return axios.get(rapidApi, { headers }).then((apiRes) => {
            const srcData = apiRes.data;
            if (!srcData)
                return [];
            const subtitles = Array.isArray(srcData.tracks)
                ? srcData.tracks
