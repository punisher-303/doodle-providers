const fs = require('fs');
const axios = require('axios');

const FILE_PATH = 'manifest.json';

// Read the manifest.json file
function readModflixJson() {
  try {
    const data = fs.readFileSync(FILE_PATH, 'utf8');
    return JSON.parse(data);
  } catch (error) {
    console.error(`Error reading ${FILE_PATH}:`, error);
    process.exit(1);
  }
}

// Extract domain (origin) from URL without trailing slash
function getDomain(sourceUrl) {
  try {
    const urlObj = new URL(sourceUrl);
    return urlObj.origin;
  } catch (error) {
    console.error(`Error parsing URL ${sourceUrl}:`, error);
    return sourceUrl;
  }
}

// Check if original URL has a trailing slash in path
function hasTrailingSlash(sourceUrl) {
  return sourceUrl.endsWith('/') && !sourceUrl.endsWith('://');
}

// Check URL and return new URL if domain redirected
async function checkUrl(sourceUrl) {
  try {
    // Set timeout to 10 seconds to avoid hanging
    const response = await axios.head(sourceUrl, {
      maxRedirects: 0,
      timeout: 10000,
      validateStatus: status => true
    });

    // If status is 200, no change needed
    if (response.status === 200) {
      console.log(`✅ ${sourceUrl} is valid (200 OK)`);
      return null;
    } else if (response.status >= 300 && response.status < 400) {
      // Handle redirects
      const newLocation = response.headers.location;
      if (newLocation) {
        // If it's a relative redirect, construct the full URL
        let fullRedirectUrl = newLocation;
        if (!newLocation.startsWith('http')) {
          const baseUrl = new URL(sourceUrl);
          fullRedirectUrl = new URL(newLocation, baseUrl.origin).toString();
        }

        console.log(`🔄 ${sourceUrl} redirects to ${fullRedirectUrl}`);

        // Get the new domain
        const newDomain = getDomain(fullRedirectUrl);

        // Check if original URL had a trailing slash
        const needsTrailingSlash = hasTrailingSlash(sourceUrl);

        // Create new URL: new domain + trailing slash if the original had one
        let finalUrl = newDomain;
        if (needsTrailingSlash) {
          finalUrl += '/';
        }

        console.log(`Will update to: ${finalUrl} (preserved trailing slash: ${needsTrailingSlash})`);
        return finalUrl;
      }
    } else {
      console.log(`⚠️ ${sourceUrl} returned status ${response.status}`);
    }
  } catch (error) {
    // Try GET request if HEAD fails
    try {
      const response = await axios.get(sourceUrl, {
        maxRedirects: 0,
        timeout: 10000,
        validateStatus: status => true
      });

      if (response.status === 200) {
        console.log(`✅ ${sourceUrl} is valid (200 OK)`);
        return null;
      } else if (response.status >= 300 && response.status < 400) {
        // Handle redirects
        const newLocation = response.headers.location;
        if (newLocation) {
          console.log(`🔄 ${sourceUrl} redirects to ${newLocation}`);

          let fullRedirectUrl = newLocation;
          if (!newLocation.startsWith('http')) {
            const baseUrl = new URL(sourceUrl);
            fullRedirectUrl = new URL(newLocation, baseUrl.origin).toString();
          }

          // Get the new domain
          const newDomain = getDomain(fullRedirectUrl);

          // Check if original URL had a trailing slash
          const needsTrailingSlash = hasTrailingSlash(sourceUrl);

          // Create new URL: new domain + trailing slash if the original had one
          let finalUrl = newDomain;
          if (needsTrailingSlash) {
            finalUrl += '/';
          }

          console.log(`Will update to: ${finalUrl} (preserved trailing slash: ${needsTrailingSlash})`);
          return finalUrl;
        }
      } else {
        console.log(`⚠️ ${sourceUrl} returned status ${response.status}`);
      }
    } catch (getError) {
      if (getError.response) {
        console.log(`⚠️ ${sourceUrl} returned status ${getError.response.status}`);
      } else if (getError.code === 'ECONNABORTED') {
        console.log(`⌛ ${sourceUrl} request timed out`);
      } else if (getError.code === 'ENOTFOUND') {
        console.log(`❌ ${sourceUrl} domain not found`);
      } else {
        console.log(`❌ Error checking ${sourceUrl}: ${getError.message}`);
      }
    }
  }

  // Return null if no change or error
  return null;
}

// Main function
async function main() {
  const providers = readModflixJson();
  let hasChanges = false;

  // Process each provider
  for (const [key, provider] of Object.entries(providers)) {
    const sourceUrl = provider.sourceUrl;
    console.log(`Checking ${provider.name} (${sourceUrl})...`);

    try {
      const newUrl = await checkUrl(sourceUrl);
      if (newUrl && newUrl !== sourceUrl) {
        provider.sourceUrl = newUrl;
        hasChanges = true;
        console.log(`Updated ${provider.name} URL from ${sourceUrl} to ${newUrl}`);
      }
    } catch (error) {
      console.log(`❌ Error processing ${sourceUrl}: ${error.message}`);
    }
  }

  // Write changes back to file if needed
  if (hasChanges) {
    // Use a space-efficient JSON format but with proper formatting
    const jsonString = JSON.stringify(providers, null, 2);
    fs.writeFileSync(FILE_PATH, jsonString);
    console.log(`✅ Updated ${FILE_PATH} with new URLs`);
  } else {
    console.log(`ℹ️ No changes needed for ${FILE_PATH}`);
  }
}

// Execute main function with error handling
main().catch(error => {
  console.error('Unhandled error:', error);
  process.exit(1);
});
