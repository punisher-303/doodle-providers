...
const getMeta = ({ link, providerContext }) => {
    ...
    return axios
        .get(link, { headers: Object.assign(Object.assign({}, headers), { Referer: baseUrl }) })
        .then(({ data }) => {
        var _a;
        ...
        // --- IMDb ID ---
        const imdbId = ((_a = ($("a[href*='imdb.com/title/']").attr("href") || "").match(/tt\d+/)) === null || _a === void 0 ? void 0 : _a[0]) || "";
        // --- IMAGE ---
        let image = body.find("img[data-src]").first().attr("data-src") ||
            body.find("img[src]").first().attr("src") ||
            "";
        if (image && image.startsWith("//"))
            image = "https:" + image;
        // --- SYNOPSIS ---
        const synopsis = body.find("h3:contains('SYNOPSIS'), h3:contains('Plot')").next("p").text().trim() ||
        ...
        body.find("h3").each((_, el) => {
            ...
            if (!/\d+p/i.test(qualityTitle))
                return;
            const quality = ((_a = qualityTitle.match(/\d+p/)) === null || _a === void 0 ? void 0 : _a[0]) || "";
            const directLinks = [];
        ...
        if (links.length === 0) {
            $("h5").each((_, el) => {
                ...
                if (!/\d+p/i.test(qTitle))
                    return;
                const quality = ((_a = qTitle.match(/\d+p/)) === null || _a === void 0 ? void 0 : _a[0]) || "";
                const directLinks = [];
                ...
                if (nextBtn.length) {
                    ...
                    const btnText = nextBtn.text().trim() ||
                ...
                downloadParagraph.find("a[href]").each((_, a) => {
                    const $a = $(a);
                    const linkHref = $a.attr("href") || "";
                    const btnText = $a.text().trim() || $a.find("button").text().trim() || ($a.attr("title") || "").trim();
