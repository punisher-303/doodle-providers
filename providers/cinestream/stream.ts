...
export const getStream = ({
  ...
  // Replaced async/await with Promise chain
  return getWebstreamerStream(
    String(imdbId),
    episode,
    season,
    effectiveType,
...
export function getWebstreamerStream(
  ...
  return providerContext.axios
    ...
    .catch((e: any) => {
      // Throwing inside a promise chain will be caught by the next catch block
      throw e;
    });
}
...
export function getRiveStream(
  ...
  // Replaced await providerContext.getBaseUrl with .then()
  return providerContext.getBaseUrl("rive").then((baseUrl: string) => {
    ...
    // Replaced Promise.all map async with standard Promise.all map
